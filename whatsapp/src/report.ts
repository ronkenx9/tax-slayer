...
import type { TaxReportData, NormalizedTransaction } from './allium.js'; // Moralis-backed data client
...
async function generatePDF(wallet: string, data: TaxReportData): Promise<string> {
  ...
  page.drawText(
    `Generated by Tax Slayer Agent via Moralis + OWS + x402  |  ${new Date().toISOString().slice(0, 10)}`,
